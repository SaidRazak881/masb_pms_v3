...
import { commitProductionBatch } from '@/lib/imports/production-commit-service'
...
export async function POST(request: Request) {
  try {
    ...
    const result = await commitProductionBatch(body.batch_id, token)
    return NextResponse.json({ data: result }, { status: 200 })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'PRODUCTION_COMMIT_FAILED'
