...
export default async function ActionCenter() {
    ...
    return (
      <div className="space-y-6 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Priority actions generated from operational data.</p>
        </div>
        <Card>
          <CardContent className="p-6">
            <h2 className="text-lg font-semibold">Unable to load Action Center</h2>
            <p className="mt-2 text-sm text-slate-600">Operational data could not be loaded right now. Please try again.</p>
  ...
  const overdueAmount = rows.reduce((sum, row) => sum + (row.category?.toUpperCase() === 'OVERDUE_INVOICE' ? Number(row.amount ?? 0) : 0), 0)
  const highPriority = rows.filter((row) => row.priority?.toUpperCase() === 'HIGH').length

  return <div className="space-y-6 p-6">
    <div><h1 className="text-2xl font-bold tracking-tight">Action Center</h1><p className="mt-1 text-sm text-slate-500">Priority actions generated from operational data.</p></div>
