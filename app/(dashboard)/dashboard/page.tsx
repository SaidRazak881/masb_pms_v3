...
async function loadDashboardData(): Promise<DashboardData> {
  ...
  const [{ count: programs, error: programsError }, { count: invoices, error: invoicesError }, { count: actions, error: actionsError }, { count: sessions, error: sessionsError }, { data: actionRows, error: actionRowsError }] = await Promise.all([
  ...
  const queryError = programsError ?? invoicesError ?? actionsError ?? sessionsError ?? actionRowsError
  if (queryError) {
  ...
  try {
    ...
    return (
      <div className="p-6">
        ...
        <DashboardOverview metrics={metrics} actions={data.actionRows} />
      </div>
    )
  } catch (error) {
