...
export default async function ImportCenterPage() {
  ...
  if (!profile || !profile.is_active || !['super_admin', 'admin', 'manager'].includes(profile.role)) redirect('/dashboard')
  ...
  return <div className="space-y-6 p-6"><div><h1 className="text-2xl font-bold tracking-tight">Import &amp; Data Quality</h1><p className="mt-1 text-sm text-slate-500">Upload workbooks, review staging batches, and monitor data quality exceptions.</p></div><ImportCenterClient batches={batches} exceptions={exceptions} /></div>
}
