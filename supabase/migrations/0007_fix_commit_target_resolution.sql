-- 0007_fix_commit_target_resolution.sql
-- Fix the "Import-to-Commit pipeline gap": `commit_import_batch` used to select
-- rows with `target_record_id is not null`, but nothing ever populated
-- `target_table`/`target_record_id` before commit, so every commit was a silent
-- no-op (affected_records = 0).
--
-- This migration:
--   1. Makes the commit predicate derive `target_table` from `source_type` when
--      it is still NULL (matching the code-side mapping in matching-engine.ts).
--   2. Removes the `target_record_id is not null` guard: for a brand-new import
--      (or an empty DB) there is no pre-existing record to reference, so the RPC
--      must be allowed to INSERT and back-fill `target_record_id` itself.
--
  ...
  for r in
    select id, source_type, normalized_data,
           coalesce(target_table,
             case source_type
               when 'quotation_tracker' then 'quotations'
               when 'invoice_2026' then 'invoices'
               when 'cost_of_sales_2026' then 'cost_of_sales'
             end) as target_table
    ...
    where batch_id=p_batch_id
      ...
      and coalesce(target_table,
            case source_type
              ...
              when 'invoice_2026' then 'invoices'
              when 'cost_of_sales_2026' then 'cost_of_sales'
            end) in ('quotations','invoices','cost_of_sales')
