...
import { Search, Bell, ChevronRight, Menu, X, UploadCloud, Plus, UserCircle2, LogOut, Command, ArrowRight } from 'lucide-react'
...
export function EnterpriseTopbar({ userEmail, userRole }: EnterpriseTopbarProps) {
  ...
  const results = React.useMemo(() => {
    ...
    if (!q) return []
    return NAV_SECTIONS.flatMap((group) => group.items.map((item) => ({ title: item.title, href: item.href, section: group.section })))
      .filter((item) => `${item.title} ${item.section}`.toLowerCase().includes(q)).slice(0, 6)
  }, [searchQuery])
  ...
  return <>
    <header className="sticky top-0 z-20 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <Link href="/dashboard" className="hidden items-center gap-2 lg:flex" aria-label="MIMOS Academy Executive Portal">
          <img src="/mimos-icon.svg" alt="MIMOS Academy" className="h-8 w-8 rounded-lg" />
          <div className="leading-tight"><p className="text-xs font-bold text-slate-900">MIMOS Academy</p><p className="text-[10px] font-semibold text-slate-500">R1/R2/R3 Executive Portal</p></div>
        </Link>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button asChild size="sm" variant="outline" className="hidden h-9 gap-1.5 border-slate-200 text-xs sm:inline-flex"><Link href="/dashboard/imports"><UploadCloud className="h-3.5 w-3.5 text-blue-600" />Upload Excel</Link></Button>
