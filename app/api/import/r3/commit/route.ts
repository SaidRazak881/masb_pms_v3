...
import { commitR3Batch } from '@/lib/imports/r3-commit-service'
...
export async function POST(request: Request) {
  try {
    ...
    const result = await commitR3Batch(body.batch_id, token)
    return NextResponse.json({ data: result }, { status: 200 })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'R3_COMMIT_FAILED'
