...
export type NavItem = { title: string; href: string; icon: React.ComponentType<{ className?: string }>; badge?: string; badgeTone?: string; superAdminOnly?: boolean }
...
export const NAV_SECTIONS: NavSection[] = [
  ...
  { section: 'DATA INTEGRITY & CONFIGURATION', items: [
    { title: 'Import & Data Quality', href: '/dashboard/imports', icon: Upload, superAdminOnly: true },
    { title: 'Reports & Export', href: '/dashboard/reports', icon: FileText },
    { title: 'System Settings', href: '/dashboard/settings', icon: Settings, superAdminOnly: true },
  ] },
]
...
export function EnterpriseSidebar({ userEmail, userRole }: { userEmail: string; userRole: string }) {
  ...
  const visibleSections = NAV_SECTIONS.map((group) => ({ ...group, items: group.items.filter((item) => !item.superAdminOnly || userRole === 'super_admin') })).filter((group) => group.items.length > 0)
  return (
    <aside className={cn('fixed inset-y-0 left-0 z-30 hidden flex-col border-r border-slate-800 bg-slate-950 text-slate-100 shadow-xl lg:flex transition-[width] duration-200', collapsed ? 'w-[72px]' : 'w-[260px]')}>
      ...
      <div className="border-t border-slate-800/80 bg-slate-900/50 p-3">
        ...
        {!collapsed && <div className="mb-2 flex items-center gap-2 px-1"><Shield className="h-3.5 w-3.5 text-emerald-400" /><div className="min-w-0"><p className="truncate text-[11px] font-semibold text-white">{userEmail || 'admin@mimos.my'}</p><p className="truncate text-[10px] font-bold uppercase tracking-wider text-slate-500">{userRole === 'super_admin' ? 'Super Admin' : userRole === 'masb_team' ? 'MASB Team' : 'Viewer'}</p></div></div>}
