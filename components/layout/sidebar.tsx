...
import { LayoutDashboard, BarChart3, FileSpreadsheet, AlertOctagon, Layers, Receipt, GraduationCap, UploadCloud, Settings, LogOut, Shield, PanelLeftClose, PanelLeftOpen } from 'lucide-react'
...
export function EnterpriseSidebar({ userEmail, userRole }: { userEmail: string; userRole: string }) {
  ...
  React.useEffect(() => {
    const sync = () => setCollapsed(window.innerWidth < 1440)
    sync()
  ...
  return (
    <aside className={cn('fixed inset-y-0 left-0 z-30 hidden flex-col border-r border-slate-800 bg-slate-950 text-slate-100 shadow-xl lg:flex transition-[width] duration-200', collapsed ? 'w-[72px]' : 'w-[260px]')}>
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-3 py-4">
        {NAV_SECTIONS.map((group) => (
          <div key={group.section}>
            {!collapsed && <p className="px-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">{group.section}</p>}
            <div className={cn('mt-2 space-y-1', collapsed && 'mt-0')}>
              {group.items.map((item) => {
                ...
                return <Link key={item.href} href={item.href} title={collapsed ? item.title : undefined} className={cn('group flex items-center rounded-lg text-xs font-semibold transition-colors', collapsed ? 'justify-center px-2 py-2.5' : 'justify-between px-3 py-2', isActive ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-900 hover:text-white')}>
                  <span className="flex min-w-0 items-center gap-2.5"><Icon className={cn('h-4 w-4 shrink-0', isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-200')} />{!collapsed && <span className="truncate">{item.title}</span>}</span>
                  {!collapsed && item.badge && <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-bold', isActive ? 'bg-white/20 text-white' : item.badgeTone ?? 'bg-slate-800 text-slate-300')}>{item.badge}</span>}
                </Link>
              })}
            </div>
          </div>
      ...
      <div className="border-t border-slate-800/80 bg-slate-900/50 p-3">
        <button type="button" onClick={() => setCollapsed((v) => !v)} className="mb-2 flex w-full items-center justify-center rounded-lg px-2 py-2 text-slate-400 hover:bg-slate-800 hover:text-white" aria-label={collapsed ? 'Kembangkan sidebar' : 'Runtuhkan sidebar'}>
          {collapsed ? <PanelLeftOpen className="h-4 w-4" /> : <><PanelLeftClose className="h-4 w-4" /><span className="ml-2 text-[11px] font-semibold">Runtuhkan menu</span></>}
        </button>
        ...
        <button type="button" onClick={handleSignOut} title={collapsed ? 'Log Keluar' : undefined} className={cn('flex w-full items-center rounded-lg border border-slate-800 bg-slate-900/80 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white', collapsed ? 'justify-center px-2' : 'justify-center gap-2 px-3')}>
          <LogOut className="h-3.5 w-3.5 text-slate-400" />{!collapsed && 'Log Keluar'}
        </button>
      </div>
    </aside>
  )
