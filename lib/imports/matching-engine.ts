...
// The import_staging.matching_status column (migration 0004/0003) uses the
...
// (the table has no matching_confidence / matching_metadata columns).
...
export async function persistMatchingResults(batchId:string, results:MatchingResult[], accessToken?:string){
  ...
  for(const result of results){
    // import_staging has no matching_confidence / matching_metadata columns in
    // the current schema, so we merge the match detail into the existing `metadata` jsonb.
