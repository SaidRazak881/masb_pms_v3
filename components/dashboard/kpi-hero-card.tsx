...
export function KpiHeroCard({ title, value, unit, subtitle, icon: Icon, tone = 'slate', trend, progress, href, badgeText, className }: KpiHeroCardProps) {
  ...
  const content = <Card className={cn('group relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:shadow-md', styles.borderHover, className)}>
    ...
    <CardContent className="p-5">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2.5"><div className={cn('flex h-10 w-10 items-center justify-center rounded-lg ring-1', styles.iconBg)}><Icon className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{title}</p>{badgeText && <span className="mt-0.5 inline-block rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-600">{badgeText}</span>}</div></div>
        {trend ? <div className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium', trend.direction === 'up' && 'bg-emerald-50 text-emerald-700', trend.direction === 'down' && 'bg-red-50 text-red-700', trend.direction === 'neutral' && 'bg-slate-100 text-slate-600')}>{trend.direction === 'up' ? <TrendingUp className="h-3 w-3" /> : trend.direction === 'down' ? <TrendingDown className="h-3 w-3" /> : <Minus className="h-3 w-3" />}{trend.value}</div> : href ? <ArrowUpRight className="h-4 w-4 text-slate-400 opacity-0 transition-opacity group-hover:opacity-100" /> : null}
      </div>
      <div className="mt-4"><div className="flex items-baseline gap-1.5"><span className={cn('text-[32px] font-semibold leading-[1.2] tracking-tight tabular-nums', styles.accent)}>{typeof value === 'number' ? value.toLocaleString('en-MY') : value}</span>{unit && <span className="text-xs font-medium text-slate-500">{unit}</span>}</div>{subtitle && <p className="mt-1 line-clamp-1 text-xs text-slate-500">{subtitle}</p>}</div>
  ...
  return href ? <a href={href} className="block no-underline focus:outline-none">{content}</a> : content
}
