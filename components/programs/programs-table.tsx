    ...
    <CardContent className="min-w-0 p-4 sm:p-6">
      <div className="mb-5 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative min-w-0 flex-1">
          ...
          <Input value={search} onChange={(event) => updateSearch(event.target.value)} placeholder="Search company or program title" className="min-w-0 pl-9" />
        </div>
        <p className="shrink-0 text-xs font-medium text-slate-500">{filteredRows.length} programs</p>
      </div>

      {/* Desktop/tablet: retain dense enterprise table with intentional horizontal scrolling. */}
          ...
          <tbody>
            {pageRows.map((row, index) => {
              ...
              return <tr key={key} className="border-t border-slate-100 odd:bg-white even:bg-slate-50/50 transition-colors hover:bg-slate-100/70">
                ...
                <td className="p-3 text-right font-medium tabular-nums">{money(row.weighted_value)}</td>
      ...
      {/* Mobile: card rows avoid forcing a 1000px table into a 320–767px viewport. */}
      <div className="space-y-3 md:hidden">
        {pageRows.map((row, index) => {
          const key = row.program_code ?? `${row.company_name}-${row.title}-${index}`
          return <article key={key} className="min-w-0 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <div className="min-w-0">
              <p className="break-words text-sm font-semibold text-blue-600">{row.program_code ?? '—'}</p>
              <h2 className="mt-1 break-words text-sm font-semibold leading-5 text-slate-900">{row.title ?? 'Untitled Program'}</h2>
              <p className="mt-2 break-words text-xs text-slate-500">{row.company_name ?? 'No company'}</p>
            </div>
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
              <StatusBadge status={stageStatus(row.current_stage)} label={row.current_stage?.replaceAll('_', ' ') ?? 'UNKNOWN'} />
              <div className="text-right text-xs font-semibold tabular-nums text-slate-800">
                <div>{money(row.forecast_value)}</div>
                <div className="mt-0.5 text-[11px] font-medium text-slate-500">Weighted {money(row.weighted_value)}</div>
              </div>
            </div>
            {row.program_code ? <div className="mt-3 grid grid-cols-2 gap-2">
              <Button asChild size="sm" variant="outline" className="h-9 text-xs"><Link href={`/dashboard/programs/${encodeURIComponent(row.program_code)}`}><ExternalLink className="mr-1.5 h-3.5 w-3.5" />View</Link></Button>
              <Button asChild size="sm" variant="outline" className="h-9 text-xs"><Link href={`/dashboard/programs/${encodeURIComponent(row.program_code)}/edit`}><Edit3 className="mr-1.5 h-3.5 w-3.5" />Edit</Link></Button>
            </div> : null}
          </article>
        })}
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex"><Button type="button" variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</Button><Button type="button" variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</Button></div>
      </div>
    </CardContent>
  </Card>
}
